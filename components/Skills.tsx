...
export default function Skills() {
  ...
  return (
    <section id="skills" className="mb-28 w-full max-w-[80rem] scroll-mt-28 text-center sm:mb-40 overflow-hidden">
      <h2 className="text-4xl font-bold mb-16 text-center text-slate-100">Tech Stack</h2>
      ...
      <div className="relative flex flex-col gap-6 overflow-hidden [mask-image:linear-gradient(to_right,transparent,black_10%,black_90%,transparent)]">
        ...
        <div className="flex w-max animate-marquee gap-6 hover:[animation-play-state:paused]">
          {[...row1, ...row1, ...row1].map((skill, index) => (
            <div
              key={index}
              className="flex-shrink-0 bg-slate-900/50 backdrop-blur-md border border-slate-700/50 rounded-2xl px-8 py-4 flex items-center justify-center min-w-[12rem] hover:border-cyan-500/50 hover:bg-slate-800/80 hover:text-cyan-400 hover:shadow-[0_0_20px_rgba(34,211,238,0.2)] transition-all duration-300 cursor-default"
            >
              <span className="text-lg font-medium text-slate-200 group-hover:text-cyan-400">{skill}</span>
            </div>
          ))}
        </div>
        ...
        <div className="flex w-max animate-marquee gap-6 [animation-direction:reverse] hover:[animation-play-state:paused]">
          {[...row2, ...row2, ...row2].map((skill, index) => (
            ...
            >
              <span className="text-lg font-medium text-slate-200 group-hover:text-pink-400">{skill}</span>
            </div>
          ))}
        </div>
